...
export const handleShowTickets = (e, bills, index, document) => {
  ...
  if (container.innerHTML === "") {
    ...
    container.innerHTML = cards(filteredBills(bills, getStatus(index)))
    
    const currentBills = filteredBills(bills, getStatus(index))
    currentBills.forEach(bill => {
      const billElement = document.querySelector(`#open-bill${bill.id}`)
      if (billElement) {
        billElement.addEventListener('click', (e) => handleEditTicket(e, bill, bills, document))
      }
    })
  } else {
